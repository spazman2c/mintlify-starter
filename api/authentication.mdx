...
<Note>
  Workspace API keys start with `woesk_`. Widget keys are public install
  identifiers and are not accepted by REST API endpoints.
</Note>
...
<Warning>
  ...
  production integration key. Playground requests may transit the docs runtime
  when proxying is enabled. For production keys, run `curl` or SDK calls from
  your own backend and revoke playground test keys after use.
</Warning>
...
- Valid REST API calls are tracked against the key that authenticated the
  request.
...
For `429 rate_limited`, back off and retry after a short delay. Public REST API
requests are rate-limited per workspace API key.

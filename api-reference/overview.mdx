...
| Section | Use it for |
| --- | --- |
| API basics | Authentication, scopes, key handling, and integration rules. |
| Resource guides | Human-readable workflow notes for each resource. |
| Endpoint groups | Generated OpenAPI pages with runnable examples and schemas. |
| Widget and surveys | Public customer-facing routes that are not authenticated with `woesk_` API keys. |

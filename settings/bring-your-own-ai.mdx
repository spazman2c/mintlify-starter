...
description: "Connect customer-owned OpenAI, Anthropic, or Google model keys for Woes support agents."
---
...
Woes supports a curated provider list:
...
Each provider entry can have one stored key. A workspace can connect multiple
providers, but only one provider is active for support-agent routing at a time.

## Frontier Model Selection

Woes exposes a curated list of frontier model ids for each provider. The model
picker prevents unsupported free-form model strings from being routed at
runtime.

If a stored or submitted model id is empty, stale, or unsupported, Woes falls
back to the default model for that provider.
...
<Steps>
  ...
  <Step title="Choose a provider">
    Select OpenAI, Claude, or Google.
  </Step>
...
- Walks through OpenAI, Claude, and Google key setup.
